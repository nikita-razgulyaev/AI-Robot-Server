...
При FAST_MODE=true в config.yaml дефолты для ltm/profile/rag — False (как и
раньше), но их всё равно можно включить из панели: тяжёлые ресурсы (Qdrant,
энкодер, файл канона) в этом случае поднимаются лениво при первом включении,
а не остаются выключенными до перезапуска.
"""
import json
import logging
import threading
